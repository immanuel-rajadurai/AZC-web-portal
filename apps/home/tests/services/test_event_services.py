...
class EventServicesTestCase(TestCase):
    def delete_test_event(self):
        ...
        events = event_services.get_events_list()
        event = next((event for event in events if event["name"] == "TEST2"), None)
        event_services.delete_event(event["id"]) if event else None

    def setUp(self):
        event_services.create_event(
            "TEST", "testing description", "https://www.google.com"
        )
    ...
    def test_edit_event(self):
        ...
        event_services.edit_event(
            event["id"], "TEST2", "testing description2", "https://www.google2.com"
        )
